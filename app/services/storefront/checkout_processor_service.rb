  ...
  class CheckoutProcessorService
    ...
    def check_presence_of_items_params
      unless @params.has_key?(:items)
        @errors[:items] = I18n.t("storefront/checkout_processor_service.errors.items")
      end
    end
    ...
    def create_order
      Order.transaction do
        ...
        line_items = @params[:items].map { |item| instantiate_line_item(item) }
        save!(line_items)
      end
    ...
    def instantiate_line_item(item)
