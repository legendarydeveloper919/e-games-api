  ...
  context "GET /coupons" do
    ...
    context "without any params" do
      ...
      it "returns 10 first Coupons" do
        get url, headers: auth_header(user)
        expected_coupons = coupons[0..9].map do |coupon|
          coupon.as_json(only: %i(id name code status discount_value max_use due_date))
        end
    ...
    context "with pagination params" do
      ...
      it "returns coupons limited by pagination" do
        get url, headers: auth_header(user), params: pagination_params
        expected_coupons = coupons[5..9].map do |coupon|
          coupon.as_json(only: %i(id name code status discount_value max_use due_date))
        end
        expect(body_json["coupons"]).to contain_exactly *expected_coupons
      end
